...
class ZodbPackMonkeyHelper(object):
    ...
    def _validate_group(self, group, to_remove, references):
        """
        For some reason, zodbpack sometimes mark objects for removal even if those objects are 
        still referenced by objects that have not been marked for removal.
        It probably has to do with a race condition between pack and modeling.
        We need to check that all oids to be removed are only referenced by other oids that have also been marked for removal.
        If we detect an oid that is referenced by an oid not marked for removal we will not delete.
        """
        valid = True
        for oid in group:
