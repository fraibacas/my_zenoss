...
class ZodbPackMonkeyHelper(object):
    ...
    '''
    def filter_oids_unproperly_marked_for_removal(self, oids_to_remove, grouped_oids_to_remove, references):
        """
        oids_to_remove: set containing oids marked to be removed (oid only, no tid)
        param grouped_oids_to_remove: list of grouped (oid, tid)
        param references: data structure containing all the relations between objects marked to be removed
        return: filtered groups
        """
        filtered_groups = []
        dropped_groups = 0
        for group in grouped_oids_to_remove:
            drop_group = False
            for oid, tid in grouped_oids_to_remove:
                refs = references.refs_from[oid]
                if any(r not in oids_to_remove for r in refs):
                    drop_group = True
                    dropped_groups = dropped_groups + 1
                    break
            if not drop_group:
    ...
    def group_oids(self, cursor, to_remove, references):
    	"""
        ...
        dump_data = {}
        try:
            with open('/opt/zenoss/log/references.pickle', 'rb') as pfile:
                dump_data = pickle.load(pfile)
        except:
            pass

        with open('/opt/zenoss/log/references.pickle', 'wb') as pfile:
            ts = str(datetime.datetime.now())
            dump_data[ts] = references
            pickle.dump(dump_data, pfile)

        return (grouped_oids_with_tid, skipped_oids)
    ...
    def run_post_pack_tests(self, cursor, to_remove, oids_not_removed):
        ...
        self._print_test_result("Validating {0}".format(table), passed)
        if not passed:
            log.info("PKE: {0} / {1} / {2}".format(r_from, r_to, removed))
    ...
    def create_pickle(self, skipped_oids, path, filename):
        ...
        try:
            with open(filename, "wb") as pfile:
                ts = str(datetime.datetime.now())
                all_skipped[ts] = skipped_oids

                pickle.dump(all_skipped, pfile)
...
try:
    ...
    def get_grouped_oids(cursor, oids_to_remove, references):
        """ Groups oids to be removed in groups of connected objects """
        group_start = time.time()
        grouped_oids = []
        try:
            grouped_oids = MONKEY_HELPER.group_oids(cursor, oids_to_remove, references)
        except Exception as e:
    ...
    def remove_isolated_oids(self, conn, cursor, grouped_oids, sleep, packed_func, total, oids_processed):
        ...
        while isolated_oids:
            ...
            if count != len(batch):
                log.error('An isolated oid was updated while pack was running. PKE?? batch/deleted:{0}/{1}'.format(len(batch), count))
                conn.rollback()
            else:
                conn.commit()
    ...
    def pack(self, pack_tid, sleep=None, packed_func=None):
        ...
        try:
            try:
                ...
                if total > 0:
                    #--------- vsphere testing
                    if MONKEY_HELPER.get_current_database(cursor) == 'zodb':

                        # --- Dump to a pickle the objects marked for removal
                        removed = {}
                        try:
                            with open('/opt/zenoss/log/zodbpack_oids_to_remove.pickle', 'rb') as pfile:
                                removed = pickle.load(pfile)
                                if isinstance(removed, list):
                                    old_data = removed
                                    removed = {}
                                    removed['old_run'] = old_data
                        except:
                            pass

                        with open('/opt/zenoss/log/zodbpack_oids_to_remove.pickle', 'wb') as pfile:
                            ts = str(datetime.datetime.now())
                            removed[ts] = to_remove
                            pickle.dump(removed, pfile)
                        #---
                    #--------- vsphere testing


                    references = MONKEY_HELPER.get_oid_references(cursor, to_remove)
                    """
                    oids_only = [ oid for (oid, tid) in to_remove ]
                    filtered_oids = MONKEY_HELPER.filer_oids_unproperly_marked_for_removal(oids_only, references)
                    unproperly_marked_oids = [ (oid, tid) for (oid, tid) in to_remove if oid not in filtered_oids ]
                    to_remove = [ (oid, tid) for (oid, tid) in to_remove if oid in filtered_oids ]

                    if unproperly_marked_oids:
                        print unproperly_marked_oids
                        MONKEY_HELPER.unmark_rolledback_oids(conn, cursor, unproperly_marked_oids)
                        log.info("Filtered {0} oids to avoid POSKeyErrors".format(total - len(to_remove)))
                    """

                    log.info("Grouping connected oids... (may take a while)")
                    grouped_oids, skipped_oids = get_grouped_oids(cursor, to_remove, references)

                    if skipped_oids:
                        log.info("Filtered {0} oids to avoid POSKeyErrors".format(len(skipped_oids)))
                        MONKEY_HELPER.unmark_rolledback_oids(conn, cursor, skipped_oids)

                    #-- test, do not remove oids that are references by oid that have not been marked for removal
                    """
                    oids_only = [ oid for (oid, tid) in to_remove ]
                    filtered_oids = MONKEY_HELPER.filer_oids_unproperly_marked_for_removal(oids_only, references)
                    if len(oids_only) != len(filtered_oids):
                        import pdb; pdb.set_trace()
                    """
                    #--

                    #-- test, do not remove isolated oids that are referenced by other oids
                    """
                    groups_to_filter = grouped_oids[:]
                    grouped_oids = []
                    isolated = []
                    skipped = []
                    for g in groups_to_filter:
                        add = True
                        if len(g) == 1:
                            isolated.append(g[0][0])
                            if references.refs_from[g[0][0]]: # No oid should reference an isolated oid marked for removal
                                skipped.append(g)
                                add = False
                        if add:
                            grouped_oids.append(g)

                    badly_skipped = set(oids_only) - set(filtered_oids)
                    badly_skipped_isolated = []
                    badly_skipped_grouped = []
                    for o in badly_skipped:
                        if o in isolated:
                            badly_skipped_isolated.append(o)
                        else:
                            badly_skipped_grouped.append(o)

                    import pdb; pdb.set_trace()
                    if skipped:
                        log.info("{0} out of {1} isolated oids should be skipped.\n {2}".format(len(skipped), len(isolated), skipped))
                        import pdb; pdb.set_trace()
                    else:
                        log.info("NO ISOLATED OIDS WERE SKIPPED")

                    #--------- vsphere testing
                    with open('/opt/zenoss/log/zodbpack_grouped_oids_to_remove.pickle', 'wb') as pfile:
                        pickle.dump(grouped_oids, pfile)
                    #--------- vsphere testing
                    """
